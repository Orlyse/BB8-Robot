...
static void i2c_reg_write(uint8_t i2c_addr, uint8_t reg_addr, uint8_t data) {
  //TODO: implement me
  uint16_t tx_buf = (data << 8) | reg_addr; 
...
void lsm303agr_init(const nrf_twi_mngr_t* i2c) {
  ...
  printf("accelerometer name: %x\n", i2c_reg_read(LSM303AGR_ACC_ADDRESS, 0x0F));
  ...
  printf("magnetometer name: %x\n", i2c_reg_read(LSM303AGR_MAG_ADDRESS, 0x4F));
...
float lsm303agr_read_temperature(void) {
  //TODO: implement me
  int16_t signed_val = (i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_TEMP_H_A) << 8) | i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_TEMP_L_A);
  
  return (((float)signed_val)/256.0) + 25.0;
}

lsm303agr_measurement_t lsm303agr_read_accelerometer(void) {
  //TODO: implement me

  int16_t xVal = (int16_t)(i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_X_H_A) << 8 | i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_X_L_A)) >> 6;
  float trueX = (float)xVal * 3.9 / 1000.0;

  int16_t yVal = (int16_t)(i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_Y_H_A) << 8 | i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_Y_L_A)) >> 6;
  float trueY = (float)yVal * 3.9 / 1000.0;

  int16_t zVal = (int16_t)(i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_Z_H_A) << 8 | i2c_reg_read(LSM303AGR_ACC_ADDRESS, OUT_Z_L_A)) >> 6;
  float trueZ = (float)zVal * 3.9 / 1000.0;

  lsm303agr_measurement_t measurement = {trueX, trueY, trueZ};
  return measurement;
}
...
lsm303agr_measurement_t lsm303agr_read_magnetometer(void) {
  ...
  int16_t xVal = (int16_t)(i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTX_H_REG_M) << 8 | i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTX_L_REG_M));
  float trueX = (float)xVal * 1.5 * 0.1;
  
  int16_t yVal = (int16_t)(i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTY_H_REG_M) << 8 | i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTY_L_REG_M));
  float trueY = (float)yVal * 1.5 * 0.1;
  
  int16_t zVal = (int16_t)(i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTZ_H_REG_M) << 8 | i2c_reg_read(LSM303AGR_MAG_ADDRESS, OUTZ_L_REG_M));
  float trueZ = (float)zVal * 1.5 * 0.1;

  lsm303agr_measurement_t measurement = {trueX, trueY, trueZ};
  return measurement;
}

float read_theta(void) {
  lsm303agr_measurement_t data = lsm303agr_read_accelerometer();
  return 180.0*atan((data.x_axis/sqrt(data.y_axis*data.y_axis + data.z_axis*data.z_axis)))/M_PI;
}

float read_psi(void) {
  lsm303agr_measurement_t data = lsm303agr_read_accelerometer();
  return 180.0*atan((data.y_axis/sqrt(data.x_axis*data.x_axis + data.z_axis*data.z_axis)))/M_PI;
}

float read_phi(void) {
  lsm303agr_measurement_t data = lsm303agr_read_accelerometer();
  return 180.0*atan((sqrt(data.x_axis*data.x_axis + data.y_axis*data.y_axis)/data.z_axis))/M_PI;
}
